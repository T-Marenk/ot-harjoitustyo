...
class ListBudget:
    ...
    def _initialize_total(
        ...
        label = ttk.Label(master=label_frame, text=f'Tämän hetkinen budjetti: {total} €')
        label.grid(
                row=0,
                column=0
        )
...
class MainView:
    ...
    def _initialize(self):
        ...
        add_income_button = ttk.Button(master=self._frame, text="Lisää tulo", command=lambda: self._handle_button('add_income'))
